...
## ⚠️ Когда Использовать Это Руководство

Если автоматическая миграция (`npm run db:expand`) не работает из-за проблем с подключением к базе данных.
...
\`\`\`sql
-- ============================================================
-- Beauty Pro CRM - Database Expansion
-- ============================================================
...
1. Нажмите кнопку **Run** (F5 или Cmd/Ctrl + Enter)
2. Дождитесь завершения (может занять 10-30 секунд)
3. Вы должны увидеть сообщение об успешном выполнении

---

## 📋 Метод 2: Проверка Существующих Таблиц

Если вы не уверены, выполнилась ли миграция, проверьте таблицы:
...
## 🔄 После Ручной Миграции

### Обновите TypeScript типы:
...
## ⚠️ Если Миграция Не Выполняется

### Проблема: "relation salons does not exist"

**Решение:** Сначала выполните базовую миграцию:

1. Откройте файл `supabase/migrations/001_initial_schema.sql`
2. Скопируйте весь SQL код
3. Вставьте в SQL Editor
4. Нажмите Run
5. После этого выполните миграцию из этого файла

### Проблема: "permission denied"

**Решение:**убедитесь, что вы используете Dashboard под правильным аккаунтом с правами администратора проекта.
...
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('clients', 'services', 'inventory_items', 'appointments');
-- Должно вернуть: 4
...
AND tablename IN ('clients', 'services', 'inventory_items', 'appointments');
...
-- Должны быть триггеры для updated_at, client_statistics, appointment_conflict
\`\`\`
...
1. ✅ Обновите TypeScript типы: `node scripts/update-types.js`
2. ✅ Запустите dev сервер: `npm run dev`
3. ✅ Откройте http://localhost:3000
4. ✅ Вы должны увидеть "Beauty Pro CRM is working!"
